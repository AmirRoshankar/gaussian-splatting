...
def convert_backgrounds(img_dir, mask_dir, dst_dir):
    for f in os.listdir(img_dir):
        ...
        mask_file = f.replace("frame_", "mask_")
...
def convert_transforms(all_tforms_path, test, dst_path):
    ...
    for f in all_tforms['frames']:
        ...
        if fname in test:
            ...
            mpath = f"./test/masks/mask_{fname[:-4].split('_')[1]}"
            test_frames.append({
                "file_path": fpath,
                "mask_path": mpath,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--already_split", "-a", action='store_true', default=False)
    args = parser.parse_args()
    already_split = args.already_split

    if not already_split:
        img_dir = os.path.join(args.source_path, "images") # copy over from mednerf
        mask_dir = os.path.join(args.source_path, "masks")  # copy over from mednerf
        transparent_dir = os.path.join(args.source_path, "input")
        train_dir = os.path.join(args.source_path, "train")
        test_dir = os.path.join(args.source_path, "test")
        all_tform_path = os.path.join(args.source_path, "transforms.json")
        ...
        convert_transforms(all_tform_path, test, args.source_path)
    elif already_split:
        subdirs = ['train', 'test']
        for subdir in subdirs:
            ...
            convert_backgrounds(mask_dir, mask_dir, mask_dir)

            # Create transform jsons
            raw_tform_path = os.path.join(subdir_path, 'transforms.json')
            dst_tform_path = os.path.join(args.source_path, f'transforms_{subdir}.json')
            convert_transforms_as(raw_tform_path, dst_tform_path) 
            os.remove(raw_tform_path)               